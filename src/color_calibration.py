...
def get_pallete_coords(image_path, template_path, template_coords,show = True):
...
def adjust_image(img,pallete_coords,radius):
	x = []
	y = []
...
def draw_circles(file_path):
	...
	pallete_coords = get_pallete_coords(image_path = file_path, template_path = "static/images/template.jpg",template_coords = [(17,87),(42,87),(67,87),(91,87),(116,87),(140,87)], show = False)
	
	radius = int(img.shape[0]*0.05)
	color = (255, 0, 0)
	thickness = int(img.shape[0]*0.01)

	img = adjust_image(img, pallete_coords,radius)

	for coord in pallete_coords:
		img = cv2.circle(img, coord, radius, color, thickness)
        
	print("color_calibration running!")
	print("palette_coords",pallete_coords)

	img_dir = 'tmp/img.jpeg'
